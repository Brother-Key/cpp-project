...
void timer(const int step)
{
    if (!paused)
    {
        // TASK_0 si le jeu n'est pas en pause on lance la simulation
        for (auto it = move_queue.begin(); it != move_queue.end(); )
        {
            auto& item = (*it);
            if (!item->move())
            {
                // Remove from move_queue
                it = move_queue.erase(it);
                // Call the destructor of item
                delete item;
            } else {
                ++it;
            }
...
void up_framerate()
{
    ...
    {
        std::cout << "FRAMERATE UP" << std::endl;
        ticks_per_sec ++;
    }
}
...
void down_framerate()
{
    if (ticks_per_sec > 1)
    {
        std::cout << "FRAMERATE DOWN" << std::endl;
        ticks_per_sec --;
    }
}
